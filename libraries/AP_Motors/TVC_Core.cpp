...
TVC_Outputs tvc_run_main_logic(const TVC_Inputs& inputs, TVC_CoreState& state, const TVC_Config& config)
{
    ...
    if (target_pitch_deg >= 0) {
        vector_pitch_out = target_pitch_deg / std::max(g_config.forward_flight_physical_angle_deg, 0.1f);
    } else {
        vector_pitch_out = target_pitch_deg / std::max(fabsf(g_config.reverse_flight_physical_angle_deg), 0.1f);
    }

    float vector_roll_out;
    if (target_roll_deg >= 0) {
        vector_roll_out = target_roll_deg / std::max(g_config.forward_flight_physical_angle_deg, 0.1f);
    } else {
        vector_roll_out = target_roll_deg / std::max(fabsf(g_config.reverse_flight_physical_angle_deg), 0.1f);
    }
#endif
    ...
    float fw_model_pitch_output = g_config.forward_flight_physical_angle_deg / g_config.forward_flight_physical_angle_deg; // 1.0f
