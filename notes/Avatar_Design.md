...
**Hold target tracking:** `_vel_hold_target = vel_bf.x` instantly whenever the stick is active and motors are not saturated. This guarantees zero velocity error at every stick release regardless of how fast the aircraft accelerated, so there is never a braking lurch when the pilot lets go. Brief-touch protection is provided by the fade factor — while the stick is active `_damp_long_fade_factor` is driven toward zero (100 ms suppression), so even if the target snaps to a drifted speed during a brief touch, the dampening output is already near zero and builds back only gradually over the 500 ms recovery. A slew rate limit is not used here: it caused systematic lag (aircraft accelerating at 1.5 m/s² would always outrun a 0.5 m/s² target) and was redundant with the fade protection.
