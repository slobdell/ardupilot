...
**No fade factor on vel-damp output:** Unlike `Q_DAMP_LONG`, the vel-damp correction is NOT multiplied by `_damp_long_fade_factor`. The fade is unnecessary because instant target tracking already guarantees zero error (and thus zero output) during stick-active periods. The dampener is always active — the zero-output condition during stick use is implicit, not enforced by a fade gate.

**Angle latch with slew (throttle-only input):** When the pilot touches the throttle with neutral pitch, the dampener zeroes (vel error → 0) but the tilt target would otherwise snap back to the uncorrected `pilot_tilt_deg`. Instead, `pilot_tilt_deg` is slewed toward `current_tilt_deg` at `Q_TILT_RATE_UP` deg/s each frame. The servo stays approximately still — the atan2 decomposition sees the combined (rising `pilot_tilt`, fading `damp_horiz`) vector remaining roughly constant. `thrust_vert` transitions smoothly rather than stepping. `Q_TILT_RATE_UP` is used as the rate not because `pilot_tilt_deg` is physically constrained by servo speed — it is a pure software variable — but because it gives a timescale perceptually coherent with servo motion and naturally matches the `_damp_long_fade_factor` 100 ms window for typical corrections. State: `_latch_tilt_start`, `_latch_tilt_target`, `_last_throttle_active`.
