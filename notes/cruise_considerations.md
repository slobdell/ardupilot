...
- **Throttle stick**: TECS owns the throttle servo; the stick is a positive-only bias. Upper half = `THROTTLE_NUDGE` (with `TRIM_THROTTLE` = 20, up to +80% — this is the pilot's **speed lever**, see § 7); lower half = deliberately nothing (stock convention: idle stick must never starve the autopilot). Bottom stick ⇒ the aircraft settles into the under-throttled mid-tilt equilibrium (motors-supported slow flight, § 7); pushing the stick up shifts the equilibrium continuously toward wing-borne cruise. Descent = pitch stick forward. **Pilot briefing: semantics invert vs STABILIZE** (STABILIZE: throttle=altitude, pitch=speed; CRUISE: pitch=altitude, throttle=speed-lever-up only). Permanent — no airspeed sensor, § 5.7.
...
1. **Entering CRUISE holds the current flight state — it is no longer a commanded transition** (`[AV-INVAR:tecs-seed-current-state]`, July 2026, compile-verified only). TECS's pitch loop is seeded from the current thrust elevation on entry: from forward flight the seed is ≈ 0° (identical to the old reset); from hover the demand opens at ≈ 90° — rotors stay vertical, throttle near `THR_MAX`, and the height loop walks tilt down toward the wing-borne (or mid-tilt Osprey) equilibrium on the `TECS_TIME_CONST` timescale, trading a gentle climb for forward speed. Previously any entry commanded tilt-to-horizontal at 30°/s regardless of state. Still: **enter CRUISE from established forward flight** until the from-hover behavior is characterized in logs.
...
**Transition walkthroughs into CRUISE** (updated for `[AV-INVAR:tecs-seed-current-state]`, § 5 #1):
- **QLOITER→CRUISE:** TECS pitch demand seeds at the current thrust elevation (hover ⇒ ≈ 90°), so tilt *holds vertical* at entry rather than slewing to horizontal; throttle blends from hover trim toward TECS's demand (near `THR_MAX` at vertical rotors, per synthetic pitch). The height loop then lowers tilt as climb evidence accumulates — the 30°/s slew now only rate-limits that walk-down, it no longer defines it. **Expect a gentle climb-and-accelerate profile — verify in first logs.**
- **STABILIZE→CRUISE:** snapshot includes the dampeners' last contribution, so vel/sink-damp fade out through the blend instead of stepping off. TECS seeds at the current tilt (mid-tilt slow flight enters as mid-tilt, forward flight as ≈ 0°); `pilot_tilt_deg` stays pinned so flipping back mid-transition is safe. Stick semantics flip at t=0 (blend = hands-grace only). Sub-note: for the first ~100 ms until TECS's 10 Hz loop first runs, `nav_pitch_cd` is stale at STABILIZE's 0 — worst case ~3° of tilt-down at the slew rate before the seeded demand takes over; negligible, but visible in high-rate logs.
...
- **`TRIM_THROTTLE` (= 20) is DELIBERATELY LOW — the default state is the mid-tilt equilibrium, not wing-borne cruise (July 2026 decision).** The no-airspeed throttle law (`AP_TECS.cpp` `_update_throttle_without_airspeed`) anchors at `nomThr = TRIM_THROTTLE + nudge`, then interpolates **up** toward `THR_MAX` proportionally to blended pitch (synth pitch = tilt elevation) over `PTCH_LIM_MAX` (90°), and **down** toward `THR_MIN` (0) for negative blended pitch over `PTCH_LIM_MIN` (−25°). Since there is no speed loop, the TRIM anchor sets what the aircraft does at bottom stick — and 20 is intentionally below the wing-borne sustain throttle (guessed ~45), so hands-off/bottom-stick CRUISE **cannot** sustain conventional forward flight: the incipient sink winds tilt up, the ramp raises throttle in step, and the aircraft parks at the mid-tilt Osprey equilibrium (see next bullet). This inverts the stock philosophy ("TRIM at desired unattended cruise") on purpose: unattended = slow, motors-supported flight; speed must be *commanded*. `THROTTLE_NUDGE` (upper half stick) is the **speed lever**: it adds to the anchor (20 → 100 across the upper half), shifting the equilibrium continuously toward wing-borne flight; releasing it lets the aircraft sag back to mid-tilt (expect a small altitude sag during the ~`TECS_TIME_CONST` re-equilibration). Rationale: pilot-authority over the flight regime, and a fail-quiet default — bottom stick can never run away into fast forward flight. History: 45 was the original wing-borne-cruise guess; the July 2026 hand test confirmed entry-from-hover throttle-up (THR_MAX end of the ramp, TRIM-independent) but showed the need for a commanded-speed scheme. To restore a wing-borne unattended cruise later, raise TRIM back toward the measured sustain throttle (walk up from 20 in flight; the two-sided caveats in this bullet still apply). Caveats: TRIM sets a throttle %, not a speed — equilibrium drifts with wind and battery sag; NOT a throttle floor — descents (pitch-forward) interpolate below the anchor toward 0, so with TRIM 20 a descent cuts throttle to near-zero quickly (glide in forward flight; `[AV-INVAR:min-thr-tilt]` floors it when rotors are vertical); the mid-tilt equilibrium sits below ~3 m/s ground speed where L1 heading lock degrades — expect heading wander at bottom stick. **Flight validation pending.**
